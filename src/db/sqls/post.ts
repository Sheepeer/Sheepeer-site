...
const GET_ALL_DRAFTS_SQL = `select * from posts join tags where isDraft = 1 and posts.tag = tag.name`

const GET_POST_SQL = `select * from posts join tags where id = ? and posts.tag = tags.name`
