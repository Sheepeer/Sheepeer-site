...
const WorkSpace = () => {
  ...
  const publishPost = (isDraft: boolean) => {
  ...
  const onImageUpload = (file: any) => {
    return new Promise((resolve, reject) => {
      axios
        .post(`${location.protocol}//${location.hostname}:36677/upload`)
        .then((res) => {
  ...
  return (
    <div className={styles["root"]}>
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        <DialogTitle>选择一个标签：</DialogTitle>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={() => publishPost(true)}>
            存草稿
          </Button>
